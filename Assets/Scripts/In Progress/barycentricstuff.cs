...
public class barycentricstuff : MonoBehaviour
{    
    ...
    void Update()
    {
        ...
        RaycastHit hit;
        Ray eyeLevelRay = new Ray(transform.position, transform.forward); 

        if (!Physics.Raycast(eyeLevelRay, out hit, Mathf.Infinity, layerMask))
        {            
            return;
        }
        else
